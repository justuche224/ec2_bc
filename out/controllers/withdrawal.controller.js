...
export class WithdrawalController {
    ...
    static async getAllCashappWithdrawals(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
    ...
    static async getAllPaypalWithdrawals(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to get all paypal withdrawals",
            }, 500);
        }
    }
    static async getAllBankWithdrawals(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to get all bank withdrawals",
            }, 500);
        }
    }
    ...
    static async createCashappWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            const result = await withdrawalService.createCashappWithdrawal(user.id, currency, amount, cashtag, cashappName);
            return c.json({ success: true, withdrawal: result }, 201);
        }
        catch (error) {
            console.error("Error creating cashapp withdrawal:", error);
            return c.json({
                error: error instanceof Error ? error.message : "Failed to create cashapp withdrawal",
            }, 500);
        }
    }
    static async createPaypalWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            const result = await withdrawalService.createPaypalWithdrawal(user.id, currency, amount, paypalEmail, paypalName);
            return c.json({ success: true, withdrawal: result }, 201);
        }
        catch (error) {
            console.error("Error creating paypal withdrawal:", error);
            return c.json({
                error: error instanceof Error ? error.message : "Failed to create paypal withdrawal",
            }, 500);
        }
    }
    static async createBankWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            const result = await withdrawalService.createBankWithdrawal(user.id, currency, amount, bankName, bankAccountNumber, bankAccountName);
            return c.json({ success: true, withdrawal: result }, 201);
        }
        catch (error) {
            console.error("Error creating bank withdrawal:", error);
            return c.json({
                error: error instanceof Error ? error.message : "Failed to create bank withdrawal",
            }, 500);
        }
    }
    ...
    static async getUserCashappWithdrawals(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to get user cashapp withdrawals",
            }, 500);
        }
    }
    static async getUserPaypalWithdrawals(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to get user paypal withdrawals",
            }, 500);
        }
    }
    static async getUserBankWithdrawals(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to get user bank withdrawals",
            }, 500);
        }
    }
    ...
    static async approveCashappWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to approve cashapp withdrawal",
            }, 500);
        }
    }
    static async approvePaypalWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to approve paypal withdrawal",
            }, 500);
        }
    }
    static async approveBankWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to approve bank withdrawal",
            }, 500);
        }
    }
    ...
    static async rejectCashappWithdrawal(c) {
        ...
        try {
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to reject cashapp withdrawal",
            }, 500);
        }
    }
    static async rejectPaypalWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to reject paypal withdrawal",
            }, 500);
        }
    }
    static async rejectBankWithdrawal(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            return c.json({
                error: error instanceof Error ? error.message : "Failed to reject bank withdrawal",
            }, 500);
        }
    }
