...
export const crowdfundingReferralStatusEnum = pgEnum("crowdfunding_referral_status", ["PENDING", "COMPLETED"]);
...
export const crowdfundingReferrals = pgTable("crowdfunding_referrals", {
    ...
    referreeId: varchar("referree_id", { length: 36 })
        .notNull()
        .references(() => user.id, { onDelete: "cascade" }),
    investmentId: varchar("investment_id", { length: 36 }).references(() => crowdfundingInvestments.id, { onDelete: "set null" }),
    status: crowdfundingReferralStatusEnum("status").notNull().default("PENDING"),
    createdAt: timestamp("created_at").notNull(),
    completedAt: timestamp("completed_at"),
});
// Crowdfunding relations
export const crowdfundingPoolsRelations = relations(crowdfundingPools, ({ many }) => ({
    investments: many(crowdfundingInvestments),
}));
export const crowdfundingInvestmentsRelations = relations(crowdfundingInvestments, ({ one }) => ({
    user: one(user, {
...
export const crowdfundingReferralsRelations = relations(crowdfundingReferrals, ({ one }) => ({
    referrer: one(user, {
