...
export class CrowdfundingController {
  ...
  static async getAllPools(c: Context) {
    try {
  ...
  static async getPool(c: Context) {
    try {
  ...
  static async createPool(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Forbidden: Admin access required" }, 403);
      ...
      const result = await crowdfundingService.createPool({
        ...validationResult.data,
        targetParticipants: validationResult.data.targetParticipants || 100,
      });
  ...
  static async updatePool(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Forbidden: Admin access required" }, 403);
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to update pool";
      const statusCode = errorMessage.includes("not found") ? 404 : 500;
      return c.json({ error: errorMessage }, statusCode);
    }
  }

  static async deletePool(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Forbidden: Admin access required" }, 403);
  ...
  static async joinPool(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
      ...
      const result = await crowdfundingService.joinPool(
        user.id,
        idValidation.data,
  ...
  static async createReferral(c: Context) {
    try {
      ...
      const statusCode =
        errorMessage.includes("must invest") ||
  ...
  static async markReferralComplete(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Forbidden: Admin access required" }, 403);
      ...
      const statusCode =
        errorMessage.includes("not found") || errorMessage.includes("already")
          ? 400
          : 500;
      return c.json({ error: errorMessage }, statusCode);
    }
